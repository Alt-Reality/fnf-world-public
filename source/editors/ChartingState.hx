...
class ChartingState extends MusicBeatState
{
	...
	var blackscreenCharting:FlxSprite;
	var undos = [];
	var redos = [];
	...
	[
		...
		['Time','mister Freeman?'],
		['Hide UI', 'Value 1: OFF/ON. Value 2: How long it takes for the tween.'],
		['Time Event'],
		['Foxyexe Event'],
		['SB Event'],
		['UE Event'],
	...
	**/
	...
	override function create()
	{
		...
		blackscreenCharting = new FlxSprite().makeGraphic(Std.int(FlxG.width * 4), Std.int(FlxG.height * 4), FlxColor.BLACK);
		blackscreenCharting.scrollFactor.set();
		blackscreenCharting.screenCenter();
		blackscreenCharting.alpha = 0.00001;
		add(blackscreenCharting);
	...
	function addSongUI():Void
	{
		...
		var loadAutosaveBtn:FlxButton = new FlxButton(110, 68, 'Load Autosave', function()//110, 68
		{
		...
		var saveEvents:FlxButton = new FlxButton(110, 38, 'Save Events', function ()
		{
			saveEvents();
		});
	...
	*/
	...
	function updateHeads():Void
	{
		...
		if(healthIconP1 == 'fredbear' && healthIconP2 == 'fredbear' && PlayState.fredSong && MainMenuState.mainweekbeat) {
			FlxG.sound.music.pause();
			vocals.pause();
			blackscreenCharting.alpha = 1;
			var challenge = new Lock.NewChallenger(FlxG.camera);
			add(challenge);
			PlayState.chartingMode = false;
			challenge.challenge('universe-end', 0, function() {}, false, true);
		}
